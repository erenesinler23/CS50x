...
bool check(char *word)
{
    ...
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        ...
        // Check if the current node has a child at the calculated index
        if (cursor->children[index] == NULL)
        {
            // If there is no child, word is not in the trie
            return false;
        }
        // Check if we have reach end of a valid word in the trie
        cursor = cursor->children[index];
    }
